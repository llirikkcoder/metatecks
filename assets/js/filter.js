/* === Фильтр товаров === */
/* Зависит от _filterConfig (определяется inline в sub_category.html) */

const attrName = _filterConfig.attrName,
      absolutePath = _filterConfig.absolutePath,
      disabledClass = '_disabled',
      isSelectedClass = 'btn--filtr--select',
      brandAttrName = _filterConfig.brandAttrName,
      brandSlugs = _filterConfig.brandSlugs;
...
$('body').on('click', '.js-filter-option', function(e){
  ...
  // Бренды с dropdown — фильтрация только через выбор модели
  if ($button.attr('data-models')) return;

  var isSelected = !($button.hasClass(isSelectedClass)),
