...
# --------------------------------------------------------------------------
# 4. Ежедневный дамп базы. До 21.08.2026 регулярных бэкапов не существовало.
# --------------------------------------------------------------------------
say "Бэкап базы -> /usr/local/bin/metateks-pg-backup + /etc/cron.d/metateks-backup"
install -m 755 "$REPO_DIR/scripts/pg_backup.sh" /usr/local/bin/metateks-pg-backup
install -m 644 "$CONF_DIR/cron-metateks-backup" /etc/cron.d/metateks-backup
mkdir -p /var/log/metateks

# --------------------------------------------------------------------------
# 5. Освобождение места (только с --reclaim).
# --------------------------------------------------------------------------
