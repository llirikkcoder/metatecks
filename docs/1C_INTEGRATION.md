...
> `/cml/...`. Клиент использует `/cml/exchange`, так что на практике важнее
> не путь, а домен — см. раздел «Простой обмена» ниже.
...
### Причина (установлена 14.09.2026 от специалиста клиента)

**В узле обмена 1С прописан домен предыдущего разработчика:**
`https://metateks.vlch.dev/cml/exchange`. Путь при этом верный — дело только в домене.

В конце августа 2026 мы намеренно отвязали `metateks.vlch.dev` от сервера: добавили
catch-all с `ssl_reject_handshake` и удалили сертификат (см. `conf/ops/README.md`).
С тех пор 1С получает отказ на этапе TLS — `tlsv1 unrecognized name`. A-запись домена
при этом до сих пор указывает на наш IP: её должен снять владелец vlch.dev.

Почему в журналах пусто: отказ происходит до HTTP, поэтому запросы не доходят ни до
Django (нет записей `Exchange`), ни до логов nginx. Внешне это выглядит как «1С вообще
не обращалась», хотя она могла исправно пытаться.

Что было до августа — по нашим данным восстановить нельзя. Домен резолвился в наш IP
и мог обслуживаться, так что причина остановки именно 26.12.2025 (в дни переезда
на новый сервер) осталась невыясненной; возможно, тогда же отключили регламентное
задание в 1С.

> **Ранее здесь была другая версия** — что 1С обращалась по корневому
> `/1c_exchange.php`, а тот отвечал 404. Она не подтвердилась: клиент использует
> `/cml/exchange`. Версия оставлена в истории коммитов, чтобы не искать её заново.

### Сопутствующая проблема (реальная, исправлена)

Независимо от домена: nginx держал увеличенные лимиты
...
Полная выгрузка каталога по нему оборвалась бы по таймауту — это было бы следующей
проблемой сразу после смены домена.
...
Проверять нужно на актуальном домене — `metateks.vlch.dev` отдаёт отказ TLS by design.

### Чек-лист восстановления обмена

Со стороны сайта всё готово, дальнейшее — на стороне 1С:

1. **сменить домен в узле обмена** — с `metateks.vlch.dev` на актуальный
   `metateks-admin.vinodesign.ru` (путь `/cml/exchange` оставить как есть);
   после переезда на боевой домен адрес сменится ещё раз, на `metateks.ru`;
2. проверить пользователя (`1c@metateks.ru`) и пароль;
3. выполнить проверку подключения;
4. запустить обмен **вручную и в режиме полной выгрузки** — при выгрузке «только
...
5. включить регламентное задание по расписанию (ранее было раз в час).
