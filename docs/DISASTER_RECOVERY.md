...
## Проверка восстановимости бэкапа (12.09.2026)

Бэкап, который никто не разворачивал, бэкапом не является. Проверка проведена
на боевом сервере: свежий дамп поднят во временную базу, число записей сверено
с боевой базой, временная база удалена.

```bash
DUMP=$(ls -t /opt/metatecks/backups/*.dump | head -1)
TMPDB="restore_check_$(date +%s)"
docker exec metateks_db psql -U metateks -d postgres -c "CREATE DATABASE $TMPDB;"
docker exec -i metateks_db pg_restore -U metateks -d "$TMPDB" --no-owner --no-privileges < "$DUMP"
# сверить: SELECT count(*) FROM catalog_product; и т.д. в обеих базах
docker exec metateks_db psql -U metateks -d postgres -c "DROP DATABASE $TMPDB;"
```

Результат 12.09.2026 (дамп `metateks_20260912_033002.dump`, 111 МБ):
все ключевые таблицы совпали — товары 22329, модели 798, характеристики 161,
заказы 6, пользователи 8, страницы 5. Восстановление проходит без ошибок.

Повторять имеет смысл раз в квартал и обязательно после смены версии Postgres.

## Расписание бэкапов: что рабочее, а что мусор

Рабочий механизм один — `/etc/cron.d/metateks-backup`, ежедневно в 3:30 запускает
`/usr/local/bin/metateks-pg-backup`, дампы складываются в `/opt/metatecks/backups`
с ротацией (хранится 14 штук, ~111 МБ каждый).

В crontab пользователя root до 12.09.2026 висели два нерабочих задания из
доконтейнерной установки, оба удалены (бэкап прежнего crontab —
`/root/crontab.backup-20260912`):

- `50 * * * * supervisorctl restart mt-celery` — supervisor на сервере не запущен,
  celery работает в контейнере `metateks_celery`; задание падало каждый час;
- `0 3 * * * /opt/metatecks/scripts/backup.sh` — у скрипта нет прав на исполнение,
  в `logs/backup.log` только `Permission denied` за все месяцы. Не путать с рабочим
  бэкапом в 3:30: это разные механизмы, и падал именно старый.

Осталось в crontab только `docker system prune` по воскресеньям — он нужен,
без него диск забивается старыми образами.
